...
% z = zeros(7);
x = [.008;.016;.032;.064;.128;.256;.512]; % 3 conditions
z = [x,x];
% z = repmat(x,[20 1]);
...
fix_param = ...
    ...
    .15; ... % Degree
    [0 0]; ... % Center
    uint8(display.white)}; % Color

%----   Region
region_param = ...
    {'none'; ... %{'none', 'exp_ring', 'rot_wedge', 'trans'}
    .5; ... % Compute size of region based on this -- FIX
    'circle'; ... %{'annulus', 'wedge', 'circle', 'rect','none'}
    [ 0 10 ]; ... % Annulus ring: [min max] of degrees
    [ 0 2*pi ]; ... % Theta: [min max] of wedge size
    [ -10 10 ]; ... % Height: [min max] in degrees
    [ -10 10 ]}; % Width: [min max] in degrees

%----  Dots
dot_param = ...
    {.1; ... % Density
    .01; ... % Kill fraction
    0; ... % Different colors (0/1)
    0; ... % Different sizes (0/1)
    .1; ... % Degree
    2; ... % Dimensionality
    1}; % Shape
...
motion_param = ...
    {2; ... % Degree per second
    'uniform'; ... %{'opposing', 'uniform'}
    pi/2; ... % Horizontal
    'uniform'; ... % Direction distribution
    {'linear_constant', 'random'}; ... %{'radial_constant', 'radial_accel', 'random', 'linear_constant', 'rotate_const_angle' , 'rotate_const_lin'}
    pi/360; ... % Rotations (rad) per frame, for constant angular speed
    'uniform'; ... % Speed distribution
    [0 0]; ... % Coherence for both-phases 
    '2-phase'; ... %{'none', '2-phase', '4-phase'}
    60; ... % Frames per period
    0.5; ... % Duty cycle
    1}; % Starting motion index
